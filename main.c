...
//  - Watchdog scratch[5] == BOOTLOADER_ENTRY_MAGIC && scratch[6] ==
//  ~BOOTLOADER_ENTRY_MAGIC
//  - No valid image header
#define BOOTLOADER_ENTRY_PIN 15
#define BOOTLOADER_ENTRY_MAGIC 0xb105f00d
#define BOOTLOADER_ENTRY_MAGIC_2 0xdeadbeef
...
#define FW_OFFSET_1 (16 * 1024)
#define FW_OFFSET_2 (1024 * 1024)
...
struct command_desc {
  ...
  uint32_t nargs;
  uint32_t resp_nargs;
  uint32_t (*size)(uint32_t *args_in, uint32_t *data_len_out,
                   uint32_t *resp_data_len_out);
  uint32_t (*handle)(uint32_t *args_in, uint8_t *data_in,
                     uint32_t *resp_args_out, uint8_t *resp_data_out);
};
...
static uint32_t handle_seal(uint32_t *args_in, uint8_t *data_in,
  ...
  struct image_header *check =
      (struct image_header *)(XIP_BASE + IMAGE_HEADER_OFFSET);
...
static enum state state_read_args(struct cmd_context *ctx) {
  ...
  uart_read_blocking(uart0, (uint8_t *)ctx->args,
...
static enum state state_handle_data(struct cmd_context *ctx) {
  ...
  if (desc->handle) {
    ctx->status =
        desc->handle(ctx->args, ctx->data, ctx->resp_args, ctx->resp_data);
    if (is_error(ctx->status)) {
      return STATE_ERROR;
    }
  } else {
  ...
  size_t resp_len = sizeof(ctx->status) +
                    (sizeof(*ctx->resp_args) * desc->resp_nargs) +
                    ctx->resp_data_len;
...
int main(void) {
  ...
  /*   char text[32];
    uart_puts(uart1, "WTD->scratch[5]: ");
    uint32_to_string(watchdog_hw->scratch[5], text);
    uart_puts(uart1, text);
    uart_puts(uart1, "WTD->scratch[6]: ");
    uint32_to_string(watchdog_hw->scratch[6], text);
    uart_puts(uart1, text);
    uart_puts(uart1, "\r\n");
    sleep_ms(100); */
  ...
  uint32_to_string(XIP_BASE + IMAGE_HEADER_OFFSET, text);
  uart_puts(uart1, text);
  uart_puts(uart1, " - ");
  ...
  // uint32_t vtor = (XIP_BASE + IMAGE_HEADER_OFFSET);
  disable_interrupts();
  reset_peripherals();
  jump_to_vtor(vtor);
  // }
  ...
  while (1) {
    ...
    case STATE_WAIT_FOR_SYNC:
      uart_puts(uart1, "wait_for_sync\n");
      state = state_wait_for_sync(&ctx);
      uart_puts(uart1, "wait_for_sync done\n");
      break;
    case STATE_READ_OPCODE:
      uart_puts(uart1, "read_opcode\n");
      state = state_read_opcode(&ctx);
      uart_puts(uart1, "read_opcode done\n");
      break;
    case STATE_READ_ARGS:
      uart_puts(uart1, "read_args\n");
      state = state_read_args(&ctx);
      uart_puts(uart1, "read_args done\n");
      break;
    case STATE_READ_DATA:
      uart_puts(uart1, "read_data\n");
      state = state_read_data(&ctx);
      uart_puts(uart1, "read_data done\n");
      break;
    case STATE_HANDLE_DATA:
      uart_puts(uart1, "handle_data\n");
      state = state_handle_data(&ctx);
      uart_puts(uart1, "handle_data done\n");
      break;
    case STATE_ERROR:
      uart_puts(uart1, "error\n");
      state = state_error(&ctx);
      uart_puts(uart1, "error done\n");
      break;
    }
  }
